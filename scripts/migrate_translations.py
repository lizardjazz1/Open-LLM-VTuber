...
def find_hardcoded_translations(
    directory: str = "src/open_llm_vtuber",
...
def extract_translation_groups(
    translations: Dict[str, List[Tuple[str, int, str]]],
    ...
    for file_path, file_translations in translations.items():
        ...
        # DEBUG: [FIXED] Remove unused variable | Ref: lint
        # current_group = {}
            ...
            if field_match:
                ...
                if field_name != current_context:
                    if current_context and context_translations:
                        ...
                        key = re.sub(
                            r"([a-z])([A-Z])", r"\1_\2", current_context
                        ).lower()
                        key = f"config.{key}"
...
def generate_replacement_code(
    context: str, key: str, translations: Dict[str, str]
) -> str:
    """
