...
class VoiceCommandParser:
    ...
    def parse_commands(self, text):
        ...
        if rate_matches or volume_matches or pitch_matches:
            logger.debug(f"🔍 Found commands in text: rate={rate_matches}, volume={volume_matches}, pitch={pitch_matches}")
            logger.debug(f"🔍 Selected commands: rate={rate_adjustment}, volume={volume_adjustment}, pitch={pitch_adjustment}")
        ...
        if volume_adjustment != "+0%":
            if not volume_adjustment.startswith('+') and not volume_adjustment.startswith('-'):
                # If no sign, add +
        ...
        if rate_adjustment != "+0%" or volume_adjustment != "+0%" or pitch_adjustment != "+0Hz":
            logger.debug(f"🎯 Final parsed parameters: rate={rate_adjustment}, volume={volume_adjustment}, pitch={pitch_adjustment}")
...
class TTSEngine(TTSInterface):
    def __init__(self, voice="en-US-AvaMultilingualNeural", rate="+0%", volume="+0%", pitch="+0Hz"):
        self.voice = voice
        self.rate = rate
        self.volume = volume
        self.pitch = pitch
    ...
    def generate_audio(self, text, file_name_no_ext=None):
        ...
        try:
            ...
            clean_text, rate_adjustment, volume_adjustment, pitch_adjustment = self.command_parser.parse_commands(text)
            
            # Log for debugging
            if rate_adjustment != "+0%" or volume_adjustment != "+0%" or pitch_adjustment != "+0Hz":
                logger.info(f"🎵 Voice commands detected: rate={rate_adjustment}, volume={volume_adjustment}, pitch={pitch_adjustment}")
            ...
            if not clean_text.strip():
                ...
                logger.info("🎵 Applying voice commands to previous text or using default")
            ...
            if not clean_text.strip() and rate_adjustment == "+0%" and volume_adjustment == "+0%" and pitch_adjustment == "+0Hz":
                logger.warning("⚠️ Both clean text and original text are empty, skipping audio generation")
                return None
            
            # If we have voice commands but no text, still generate audio with a space
            if not clean_text.strip() and (rate_adjustment != "+0%" or volume_adjustment != "+0%" or pitch_adjustment != "+0Hz"):
                logger.info("🎵 Generating audio with voice commands but no text - using dot")
                clean_text = "."
            
            # Log final parameters for diagnosis
            logger.info(f"🎯 Final TTS parameters: rate={rate_adjustment}, volume={volume_adjustment}, pitch={pitch_adjustment}")
            
            # Log file path
            logger.info(f"📁 Generating audio file: {file_name}")
            ...
            logger.info(f"🎤 Edge TTS communicate object created with voice: {self.voice}")
            logger.info(f"📝 Text to synthesize: '{clean_text}'")
            ...
            if not os.path.exists(file_name):
                ...
                logger.info(f"✅ Audio file created successfully: {file_name} ({os.path.getsize(file_name)} bytes)")
