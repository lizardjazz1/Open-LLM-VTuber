...
class ServiceContext:
    ...
    async def _init_mcp_components(self, use_mcpp, enabled_servers):
        ...
        if use_mcpp and enabled_servers:
            ...
            try:
                ...
                logger.info(
                    t("service.dynamically_generated_mcp_prompt", length=len(self.mcp_prompt))
                )
                logger.info(
                    t("service.dynamically_formatted_tools", openai=len(openai_tools), claude=len(claude_tools))
                )
                ...
                logger.error(
                    t("service.failed_dynamic_mcp_tool_construction", error=str(e)), exc_info=True
                )
            ...
            if self.mcp_server_registery:
                ...
                logger.error(
                    t("service.mcp_enabled_but_server_registry_not_available")
                )
            ...
            if self.mcp_client and self.tool_manager:
                ...
                logger.warning(
                    t("service.mcp_client_or_tool_manager_not_available")
                )
            ...
            logger.debug(
                t("service.mcp_components_not_initialized")
            )
    ...
    async def close(self):
        ...
        if self.mcp_client:
            logger.info(t("service.closing_mcp_client_for_context_instance", context_id=id(self)))
    ...
    async def load_cache(
        ...
        logger.debug(t("service.loaded_service_context_with_cache", character_config=character_config))
    ...
    async def load_from_config(self, config: Config) -> None:
        ...
        if (
            ...
            if not self.mcp_server_registery:
                logger.info(
                    t("service.initializing_shared_server_registry_within_load_from_config")
                )
                self.mcp_server_registery = ServerRegistry()
            logger.info(t("service.initializing_shared_tool_adapter_within_load_from_config"))
    ...
    def init_live2d(self, live2d_model_name: str) -> None:
        ...
        try:
            ...
            logger.info(t("service.live2d_initialized", info=self.live2d_model.model_info))
        except Exception as e:
    ...
    async def init_agent(self, agent_config: AgentConfig, persona_prompt: str) -> None:
        ...
        logger.info(t("service.initializing_agent", agent=agent_config.conversation_agent_choice))
    ...
    def init_translate(self, translator_config: TranslatorConfig) -> None:
        ...
        if (
            ...
            logger.info(
                t("service.initializing_translator", provider=translator_config.translate_provider)
            )
    ...
    async def handle_config_switch(
        ...
        try:
            ...
            if new_character_config_data:
                ...
                logger.info(t("service.configuration_switched", config_file_name=config_file_name))
            else:
                raise ValueError(
                    t("service.failed_to_load_configuration_from", config_file_name=config_file_name)
                )
